...
5. Observability is built in: Sentry captures 500-level exceptions, Prometheus scrapes `/api/metrics`, and Grafana is provisioned with Prometheus as the default datasource.
...
- Verify health at `/api/v1/health`, metrics at `/api/metrics`, and logs in Sentry/Grafana.
