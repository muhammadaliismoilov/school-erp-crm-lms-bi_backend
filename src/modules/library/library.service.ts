...
export class LibraryService {
  constructor(@InjectRepository(LibraryBook) private readonly books: Repository<LibraryBook>, @InjectRepository(LibraryBookCopy) private readonly copies: Repository<LibraryBookCopy>, @InjectRepository(LibraryLoan) private readonly loans: Repository<LibraryLoan>, @InjectRepository(LibraryReservation) private readonly reservations: Repository<LibraryReservation>, private readonly tenant: TenantContextService) {}

  findBooks() { return this.books.find({ where: tenantWhere<LibraryBook>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findCopies() { return this.copies.find({ where: tenantWhere<LibraryBookCopy>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findLoans() { return this.loans.find({ where: tenantWhere<LibraryLoan>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findReservations() { return this.reservations.find({ where: tenantWhere<LibraryReservation>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
