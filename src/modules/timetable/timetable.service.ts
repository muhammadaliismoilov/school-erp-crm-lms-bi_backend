...
export class TimetableService {
  constructor(@InjectRepository(TimetableTemplate) private readonly templates: Repository<TimetableTemplate>, @InjectRepository(TimetableSlot) private readonly slots: Repository<TimetableSlot>, @InjectRepository(TimetableSubstitution) private readonly substitutions: Repository<TimetableSubstitution>, @InjectRepository(TimetableConflict) private readonly conflicts: Repository<TimetableConflict>, private readonly tenant: TenantContextService) {}

  findTemplates() { return this.templates.find({ where: tenantWhere<TimetableTemplate>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findSlots() { return this.slots.find({ where: tenantWhere<TimetableSlot>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findSubstitutions() { return this.substitutions.find({ where: tenantWhere<TimetableSubstitution>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findConflicts() { return this.conflicts.find({ where: tenantWhere<TimetableConflict>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
