...
export class HealthSafetyService {
  constructor(@InjectRepository(StudentHealthRecord) private readonly records: Repository<StudentHealthRecord>, @InjectRepository(NurseVisit) private readonly nursevisits: Repository<NurseVisit>, @InjectRepository(SafetyIncident) private readonly incidents: Repository<SafetyIncident>, @InjectRepository(EmergencyDrill) private readonly drills: Repository<EmergencyDrill>, private readonly tenant: TenantContextService) {}

  findRecords() { return this.records.find({ where: tenantWhere<StudentHealthRecord>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findNurseVisits() { return this.nursevisits.find({ where: tenantWhere<NurseVisit>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findIncidents() { return this.incidents.find({ where: tenantWhere<SafetyIncident>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
  ...
  findDrills() { return this.drills.find({ where: tenantWhere<EmergencyDrill>(this.tenant, {}, { branch: true }), order: { createdAt: 'DESC' } }); }
